...
st.markdown(
    " 범행 증거를 찾아내는"
    "성능을 비교한다. 질의 200개, 후보 청크 15,522개.")
st.divider()
...
st.dataframe(pd.DataFrame([
    ...
    ["42508", "00:22:38", "target → troy",
     "Mueller Inc Foursquare Healthcare find out that with these offices there "
     "you have them on contact they have a panic there they are ready to pay"],
], columns=["행번호", "시각", "발신 → 수신", "body_en"]),
    hide_index=True, use_container_width=True)

st.info("대화 상대 쌍 + 1시간 공백으로 세션을 묶어 "
        "15,522개 청크로 만들었다. 분할 기준은 영문이라 모든 언어가 같은 경계로 "
        "잘리고 청크 번호가 일치한다.")
